...
import { AlertTriangle, FolderPlus, MapPlus } from "lucide-react";
...
export default function BrowserSection({ id, path }: BrowserSectionProps) {
  ...
  return (
    <div className="flex-1 min-h-full flex flex-col gap-y-4 md:gap-y-8 p-4 md:p-8">
      ...
      >
        {/* WARNING BANNER */}
        <div className="w-full bg-warning text-warning-foreground border border-warning rounded-sm p-4 flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-start gap-3 flex-1">
            <AlertTriangle className="size-5 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="text-sm">
                Personal workspaces are currently deprecated, and will be
                deleted in the future. We recommend you to use a personal
                organization instead. If you don't have one yet, you can create
                one{" "}
                <Link
                  href="/organizations/new?personal=true"
                  className="underline"
                >
                  here
                </Link>
